...
// parseCourseCmd represents the parseCourse command
var parseCourseCmd = &cobra.Command{
	Use:   "parseCourse",
...
- Class: mult_question
  ...
  Hint: Mapping to the transcriptome uses only the final sequences of all known transcripts from a given genome
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// print message
		fmt.Println("Parsing lesson.yaml")

		// filename argument- should this go under the init?
		var fileName string
...
func init() {
	rootCmd.AddCommand(parseCourseCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// parseCourseCmd.PersistentFlags().String("foo", "", "A help for foo")

	// file flag
	parseCourseCmd.PersistentFlags().String("file", "f", "lesson.yaml file to be parsed")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// parseCourseCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
